...
class BddTransaction(object):
    ...
    def configCon(self, fichierConfig='infodb.ini'):
        try:
        ...
        for tables in bddXml:
            ...
            for v in values:
                ...
                if(len(tables[1]) > 1):
                    ...
                    query = ("""INSERT INTO {table}({column}) VALUES(%s);""").format(table=tables[0], column=column) #Dans la requete slq, les données ne sont pas dans un tuple d'où (%s)
                    vlue = str(v[0])
                    try:
                        cur.execute(query, (vlue, ))
                    except(psycopg2.errors.UniqueViolation):
